...
Cell.prototype.drawTop = function(drawRight){
  ...
  if(this.cells){  //TODO: cells should always exist - some tests don't fill it in though
	this.widths.forEach(function(width,index){
	  ...
	  content.push(
		utils.repeat(this.chars[this.y == 0 ? 'top' : 'mid'],width,true)
	  );
	},this);
  }
  else {
	content.push(this._topLeftChar(0));
	content.push(utils.repeat(this.chars[this.y == 0 ? 'top' : 'mid'],this.width,true));
  }
  if(drawRight){
...
Cell.prototype.drawLine = function(lineNum,drawRight,forceTruncationSymbol,spanningCell){
  ...
  let w = utils.stringWidth(this.chars[this.y == 0 ? 'top' : 'mid']);
  if (w) len += this.width % w;
...
Cell.prototype.drawBottom = function(drawRight){
  ...
  var content = utils.repeat(this.chars.bottom,this.width, true);
